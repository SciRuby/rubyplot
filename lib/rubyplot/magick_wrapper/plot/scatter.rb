  ...
  module MagickWrapper
    module Plot
      class Scatter < Artist
        ...
        # This is useful when working with a small range of high values
        # (for example, a date range of months, while seconds as units)
        ...
        def data(x_data_points=[], y_data_points=[], label: :default, color: :default)
          ...
          @geometry.maximum_x_value = if x_data_points.max > @geometry.maximum_x_value then x_data_points.max
                                      else @geometry.maximum_x_value
                                      end
          @geometry.minimum_x_value = if x_data_points.min < @geometry.minimum_x_value then x_data_points.min
                                      else @geometry.minimum_x_value
                                      end
        end
        ...
        def draw_line_markers!
          ...
          if @geometry.x_axis_increment.nil?
            ...
            @x_increment = significant(@x_increment) unless @geometry.disable_significant_rounding_x_axis
          else
          ...
          (0..@geometry.marker_x_count).each do |index|
            # TODO: Fix the vertical lines, and enable them by default.
            # Not pretty when they don't match up with top y-axis line
