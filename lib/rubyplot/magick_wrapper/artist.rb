...
module Rubyplot
  module MagickWrapper
    class Artist
      ...
      def marker_size=(marker_size); end

      def marker_type=(marker_type); end
      ...
      def data(data_points=[], label: :default)
        # color: :default is removed as it is unused.
      ...
      def setup_graph_measurements
        ...
        @title_caps_height =if @geometry.hide_title || @axes.title.nil? then 0
                            else calculate_caps_height(@title_font_size) * @axes.title.lines.to_a.size
                            end
        ...
        longest_left_label_width = if @geometry.has_left_labels
                                     calculate_width(
                                       @marker_font_size,
        ...
        line_number_width =if @geometry.hide_line_numbers && !@geometry.has_left_labels then 0.0
                           else longest_left_label_width + LABEL_MARGIN * 2
                           end
        ...
        extra_room_for_long_label =if last_label >= (@geometry.column_count - 1) && @geometry.center_labels_over_point
                                     calculate_width(@marker_font_size,@axes.x_ticks[last_label]) / 2.0
                                   else 0
                                   end
        ...
        @graph_top =if @geometry.legend_at_bottom then @geometry.top_margin
                    else (@geometry.top_margin + if @geometry.hide_title then @axes.title_margin
                                                 else @title_caps_height + @axes.title_margin
                                                 end + (@legend_caps_height + legend_margin))
                    end

        x_axis_label_height = @geometry.x_axis_label .nil? ? 0.0 : @marker_caps_height + LABEL_MARGIN
      ...
      def draw_axis_labels!
        unless @geometry.x_axis_label.nil?
          ...
          @d = @d.scale_annotation(@base_image, @geometry.raw_columns, 1.0, 0.0, x_axis_label_y_coordinate,
        ...
        unless @geometry.y_axis_label .nil?
          ...
          @d = @d.scale_annotation(@base_image, 1.0, @raw_rows, @geometry.left_margin + @marker_caps_height / 2.0, 0.0,
      ...
      def draw_legend!
        ...
        @legend_labels.each do |label|
          ...
          label_widths.push [label_widths.last.pop] if sum(label_widths.last) > (@geometry.raw_columns * 0.9)
        end

        current_x_offset = center(sum(label_widths.first))
        current_y_offset = if @geometry.legend_at_bottom then @graph_height + @axes.title_margin
                           elsif @geometry.hide_title then @geometry.top_margin + @axes.title_margin
                           else @geometry.top_margin + @axes.title_margin + @title_caps_height
                           end
        ...
        @legend_labels.each_with_index do |legend_label, index|
          ...
          @d = @d.fill('black')
          @d = @d.fill(@plot_colors[index]) if defined? @plot_colors
          @d = @d.rectangle(current_x_offset,
            current_y_offset - legend_square_width / 2.0,
            current_x_offset + legend_square_width,
            current_y_offset + legend_square_width / 2.0)
          # string = 'hello' + index.to_s + '.png'
      ...
      def draw_line_markers!
        ...
        (0..@geometry.marker_count).each do |index|
          ...
          @d = @d.line(@graph_left, y, @graph_right, y) if
            !@geometry.hide_line_markers || index.zero?
      ...
      def draw_label(x_offset, index)
        y_offset = @graph_bottom + LABEL_MARGIN if !@axes.x_ticks[index].nil? && @geometry.labels_seen[index].nil?
      ...
      def label_string(value, increment)
        label =
          ...
          elsif (@spread.to_f % (@geometry.marker_count.to_f.zero? ? 1 : @geometry.marker_count.to_f)).zero? ||
